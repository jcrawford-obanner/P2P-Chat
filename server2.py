...
def multi_threaded_client(c,addr):
  ...
  while True:
    ...
    data = data[1:]
    print('reply: ',data)
    if status_dict[data] == "online":
      try:
        c.send(str.encode('{} {} {}'.format(addr[0], addr[1], 50055)))
        print("Information successfully sent to client...")
        print("Listening for new connections...")
      except:
        raise Exception("Error sending msg to client")
    else:
      c.send(str.encode('This user is currently offline. Any messages sent will be saved and sent when they log on again'))
      data = c.recv(1024).decode()
    break
